...
The generator lives at `tools/generate/`. It reads `testing/schema.graphql` and `tools/generate/config.json` and emits Go source into `jamfprotect/`.
...
- **Static-emitted files** — driven by `config.json` `statics` array. Source lives in `tools/generate/_static/`. Edit there, then run `make generate` to copy to `jamfprotect/`.
